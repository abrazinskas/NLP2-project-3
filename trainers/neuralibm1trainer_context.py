...
class NeuralIBM1ContextTrainer:
  """
  ...
  def train(self):
    ...
    for epoch_id in range(1, self.num_epochs + 1):
      ...
      for batch_id, batch in enumerate(iterate_minibatches(
        ...
        source, target = prepare_data(batch, self.model.x_vocabulary, self.model.y_vocabulary)

        # create history
        history = create_history(target)
      ...
      # save parameters
      save_path = self.model.save(self.session, path="../tmp/model.ckpt")
